...
#define CHECK_TAG(x, value)	((x) == str2long((guchar *)value))
#define RGB_FLAG     1
#define ALPHA_FLAG   2
 ...
 */
{
  ...
  gimp_install_procedure ("file_maya_iff_load",
			  ident,
			  "loads files of the Autodesk Maya IFF format",
			  "Michael Taylor",
			  "Michael Taylor",
			  "1999,2001",
			  "<Load>/Autodesk MayaIFF",
			  NULL,
			  GIMP_PLUGIN,
  ...
  gimp_install_procedure ("file_maya_iff_save",
			  ident,
                          "save file in the Autodesk Maya's IFF format",
                          "Michael Taylor",
                          "Michael Taylor",
                          "2001",
                          "<Save>/Autodesk MayaIFF",
                          "RGB*, GRAY*",
                          GIMP_PLUGIN,
                          nsave_args, 0,
                          save_args, NULL);
 ...
 */
{
  ...
  /* FIXME 2006-01-17T14:49:51.00+9:00 */
  /* assert (result == 1); */
...
#if 0
guchar long2str_buf[5];

static guchar *long2str (guint32 value)
{
  long2str_buf[0] = (guchar) (( value >> 24 ) % 256);
  long2str_buf[1] = (guchar) (( value >> 16 ) % 256);
  long2str_buf[2] = (guchar) (( value >>  8 ) % 256);
  long2str_buf[3] = (guchar) (value % 256);
  long2str_buf[4] = 0;
...
static guint32 str2long (guchar *buf)
{
  return (buf[0] << 24) + (buf[1] << 16) + (buf[2] << 8) + (buf[3] << 0);
}
 ...
 */
{
    ...
    if ( CHECK_TAG (chunkStack[chunkDepth].tag, "FOR4"))
      {
...
static void begin_write_chunk( FILE * file, gchar* tag_str, gchar* type_str)
{
  /*
   *  Description:
  ...
  if (tag_str)
    tag = (guint32) ((tag_str[0]<<24)
		     + (tag_str[1]<<16)
		     + (tag_str[2]<<8)
		     + tag_str[3]);
  if (type_str)
    type= (guint32) ((type_str[0]<<24)
		     + (type_str[1]<<16)
		     + (type_str[2]<<8)
		     + type_str[3]);
 ...
 */
 {
    gint state = 0;
    gint index = 0;
    gint runCount = 0, i, last;
    guchar nextChar, firstChar = 0, lastChar = 0, run[128];
    guint32 destIndex = 0; 
 ...
 */
{
  ...
  GimpDrawable * z_drawable = NULL;
  gint32      image_ID;
  gint32      layer_ID, z_layer_ID;
  GimpPixelRgn   pixel_rgn, z_pixel_rgn;
  iff_chunk   chunkInfo;
  ...
  GimpImageBaseType  imgtype = GIMP_RGB;
  ...
  while (TRUE)
    {
      ...
      if (CHECK_TAG (chunkInfo.tag, "TBHD"))
	{
	  /* Header chunk found */
	  width = get_long (file);
  ...
  if (flags & ZBUFFER_FLAG)
    {
      z_layer_ID = gimp_layer_new (image_ID, "Z-Buffer", width, height,
  ...
  while (TRUE)
    {
      ...
      if ( CHECK_TAG (chunkInfo.chunkType, "TBMP"))
	{
	  /* Image data found */
	  guint16 x1, x2, y1, y2, reverse_y, tile_width, tile_height;
	  guint32 zmin = 0, zmax = 0;
	  gdouble znormalfactor = 0;
	  guint32 tile = 0;
	  guint32 ztile = 0;
	  size_t fpos = 0;
	  ...
	  while ((tile < tiles ) || ( ztile < tiles ))
	    {
	      chunkInfo = begin_read_chunk( file );
	      assert(CHECK_TAG (chunkInfo.tag, "RGBA")
		     || CHECK_TAG (chunkInfo.tag, "ZBUF"));
	      ...
	      if (chunkInfo.size >= (tile_width * tile_height * depth + 8))
		{
	      ...
	      if ( is_zbuffer_normization_pass )
		{
		   ...
		   * normalize it 
		   */
		  if (CHECK_TAG (chunkInfo.tag, "ZBUF"))
		    {
		      end_read_chunk( file );
		      continue;
                    }
	      ...
	      if ( CHECK_TAG (chunkInfo.tag, "RGBA") )
		{
	      ...
	      else if ( CHECK_TAG (chunkInfo.tag, "ZBUF"))
		{
 ...
 */
{
  ...
  begin_write_chunk( file, "FVER", NULL );
  put_short( 1, file ); 
  put_short( 1, file ); 
  end_write_chunk( file );
    
  /* Write the image header */
  begin_write_chunk( file, "TBHD", NULL );
  ...
  /* Write Fields Block */
  begin_write_chunk( file, "FLDS", NULL );
  begin_write_chunk( file, "FMT\001", NULL );
  ...
  for ( row = ( tiles_in_y - 1 ); row >= 0; --row ) {
	...
	tile_height = tile_size;
      ...
      begin_write_chunk( file, "RGBA", NULL );
      x1 = (guint16)tile_x;
